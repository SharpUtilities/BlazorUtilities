...
/// </summary>
internal static class BlazorStateEventWiringHelper
{
    public static void WireUpEvents(IServiceProvider services)
    {
        var dispatcher = services.GetRequiredService<BlazorStateEventDispatcher>();
        var options = services.GetRequiredService<IOptions<BlazorStateEventOptions>>().Value;
