...
internal sealed partial class BlazorStateCleanupService : BackgroundService
{
    ...
    private void RaiseExpirationEvents(ExpiredBlazorStateInfo entry)
    {
        try
        {
            _eventDispatcher.RaiseValueCleared(entry.SessionKey, entry.StateType, entry.Value);
            _eventDispatcher.RaiseBackendOperation(entry.SessionKey, entry.StateType, BlazorStateBackendOperation.Expired);
        }
        catch (Exception ex)
        {
