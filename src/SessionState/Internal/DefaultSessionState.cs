...
/// <summary>
/// Default implementation of <see cref="SessionState"/>.
    ...
    where T : class, ISessionStateType<T>
{
    ...
    public DefaultSessionState(
        ...
        ISessionStateKeyAccessor keyAccessor,
        ILogger<DefaultSessionState<T>> logger,
        SessionStatePipelineMarker pipelineMarker,
        SessionStateEventDispatcher eventDispatcher,
        IOptions<SessionStateEventOptions> eventOptions)
    {
        _cache = cache;
        _backend = backend;
    ...
    public override async ValueTask<bool> SetAsync(T value, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (_eventOptions.TrackPropertyChanges)
            {
    ...
    private string GetRequiredSessionKey()
    {
        ...
        if (key is null)
        {
            throw new InvalidOperationException(
                ...
                "Otherwise, ensure UseBlazorSessionState() middleware has run.");
        }
    ...
    private async ValueTask LoadCoreAsync(string key, CancellationToken cancellationToken)
    {
        ...
        if (_eventOptions.TrackPropertyChanges && value is not null)
        {
            _cache.Set(key, value, CreatePropertyChangedHandler(key));
        }
        else
    ...
    private PropertyChangedEventHandler CreatePropertyChangedHandler(string sessionKey)
    {
        return (sender, args) =>
        {
            ...
            if (_eventOptions.AutoPersistOnPropertyChange)
            {
    ...
    [LoggerMessage(LogLevel.Debug, "SessionState<{TypeName}> set for key {Key}")]
    partial void LogStateSet(string typeName, string key);

    [LoggerMessage(LogLevel.Debug, "SessionState<{TypeName}> cleared for key {Key}")]
    partial void LogStateCleared(string typeName, string key);

    [LoggerMessage(LogLevel.Debug, "SessionState<{TypeName}> set skipped - values are equal")]
    partial void LogSkippedSetDueToEquality(string typeName);

    [LoggerMessage(LogLevel.Debug, "SessionState<{TypeName}> cache invalidated by external change")]
    partial void LogCacheInvalidated(string typeName);

    [LoggerMessage(LogLevel.Error, "Error auto-persisting SessionState<{TypeName}>")]
    partial void LogAutoPersistError(string typeName, Exception ex);

    [LoggerMessage(LogLevel.Error, "Error in SessionState<{TypeName}> Changed event handler")]
