...
/// </summary>
internal sealed partial class SessionStateEventDispatcher : ISessionStateEventDispatcher
{
    ...
    [LoggerMessage(LogLevel.Debug, "SessionState event {eventName} raised for {typeName} (key: {key})")]
    partial void LogEventRaised(string eventName, string typeName, string key);

    [LoggerMessage(LogLevel.Debug, "SessionState backend event {operation} for {typeName} (key: {key})")]
    partial void LogBackendEventRaised(string operation, string typeName, string key);

    [LoggerMessage(LogLevel.Error, "Error in SessionState event handler")]
