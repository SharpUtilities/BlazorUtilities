...
/// </summary>
public interface ISessionStateLockManager
{
    /// <summary>
    ...
    bool TryAcquireLock(string name, out SessionStateLock lockHandle);
    ...
    bool TryAcquireLock(string name, TimeSpan timeout, out SessionStateLock lockHandle);
    ...
    ValueTask<SessionStateLock> AcquireLockAsync(string name, CancellationToken cancellationToken = default);
    ...
    /// <param name="timeout">Maximum time to wait for the lock.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The lock handle. Check <see cref="SessionStateLock.IsAcquired"/> to see if successful.</returns>
    ValueTask<SessionStateLock> TryAcquireLockAsync(string name, TimeSpan timeout, CancellationToken cancellationToken = default);
