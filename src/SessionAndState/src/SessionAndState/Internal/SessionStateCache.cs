...
/// </summary>
internal sealed class SessionStateCache<T> : IDisposable
    where T : class, ISessionStateType<T>
{
    private readonly SessionStateEventDispatcher _eventDispatcher;
    private readonly EventHandler<SessionStateBackendEventArgs> _backendOperationHandler;
    ...
    public SessionStateCache(SessionStateEventDispatcher eventDispatcher)
    {
    ...
    private void OnBackendOperation(object? sender, SessionStateBackendEventArgs args)
    {
        if (_disposed)
        {
            return;
        ...
        if (args.Operation is not (SessionStateBackendOperation.Set
            or SessionStateBackendOperation.Remove
            or SessionStateBackendOperation.Expired
            or SessionStateBackendOperation.SessionCleared))
        {
            return;
        }
