...
namespace SessionAndState.Internal.Backends;

/// <summary>
/// In-memory implementation of ISessionStateBackend.
/// Suitable for single-server deployments.
/// </summary>
internal sealed partial class InMemorySessionAndStateBackend : ISessionAndStateBackend
{
    private readonly ConcurrentDictionary<StorageKey, SessionStateEntry> _store = new();
    private readonly SessionAndStateTimeProvider _timeProvider;
    private readonly ILogger<InMemorySessionAndStateBackend> _logger;

    public InMemorySessionAndStateBackend(
        SessionAndStateTimeProvider timeProvider,
        ILogger<InMemorySessionAndStateBackend> logger)
    {
        ...
        where T : class, ISessionStateType<T>
    {
        ...
        _store.AddOrUpdate(
            ...
            (_, existing) => new SessionStateEntry
            {
                Value = value,
    ...
    public ValueTask<IReadOnlyList<ExpiredSessionStateInfo>> RemoveExpiredAsync(CancellationToken cancellationToken = default)
    {
        var now = _timeProvider.GetUtcNow();
        var expiredEntries = new List<ExpiredSessionStateInfo>();
        ...
        foreach (var kvp in _store)
        {
            if (kvp.Value.IsExpired(now))
            {
                ...
                {
                    expiredEntries.Add(new ExpiredSessionStateInfo
                    {
        ...
        return ValueTask.FromResult<IReadOnlyList<ExpiredSessionStateInfo>>(expiredEntries);
    }

    public ValueTask<IReadOnlyList<RemovedSessionStateInfo>> RemoveAllForKeyAsync(string sessionKey, CancellationToken cancellationToken = default)
    {
        var removedEntries = new List<RemovedSessionStateInfo>();
        ...
        foreach (var key in _store.Keys)
        {
            if (key.SessionKey == sessionKey)
            {
                ...
                {
                    removedEntries.Add(new RemovedSessionStateInfo
                    {
        ...
        return ValueTask.FromResult<IReadOnlyList<RemovedSessionStateInfo>>(removedEntries);
    }
