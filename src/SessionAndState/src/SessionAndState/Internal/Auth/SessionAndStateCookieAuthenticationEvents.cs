...
namespace SessionAndState.Internal.Auth;

/// <summary>
/// Cookie authentication events that integrate with SessionAndState.
/// Handles session key injection during sign-in.
/// Wraps any existing events configured by the application.
/// </summary>
internal sealed class SessionAndStateCookieAuthenticationEvents : CookieAuthenticationEvents
{
    private readonly CookieAuthenticationEvents? _innerEvents;
    private readonly Type? _innerEventsType;

    public SessionAndStateCookieAuthenticationEvents(
    ...
    private async Task InjectSessionKeyClaimAsync(CookieSigningInContext context)
    {
        ...
        var sessionManager = services.GetRequiredService<ISessionAndStateSessionManager>();
    ...
    public override async Task ValidatePrincipal(CookieValidatePrincipalContext context)
    {
        ...
        if (context.Principal is not null)
        {
            var sessionManager = context.HttpContext.RequestServices
                .GetRequiredService<ISessionAndStateSessionManager>();
    ...
    private async Task ClearSessionAsync(CookieSigningOutContext context)
    {
        ...
        {
            var blazorContext = services.GetService<SessionStateContext>();
        ...
        var backend = services.GetRequiredService<ISessionAndStateBackend>();
        ...
        var eventDispatcher = services.GetRequiredService<SessionStateEventDispatcher>();
        foreach (var entry in removedEntries)
        {
            eventDispatcher.RaiseValueCleared(entry.SessionKey, entry.StateType, entry.Value);
            eventDispatcher.RaiseBackendOperation(entry.SessionKey, entry.StateType, SessionStateBackendOperation.Remove);
        }
