...
/// </summary>
/// <typeparam name="T">
/// The type of state to store. Must implement <see cref="ISessionStateType{T}"/>.
/// </typeparam>
public abstract class SessionState<T> where T : class, ISessionStateType<T>
{
    /// <summary>
    /// Internal constructor to prevent external inheritance.
    /// </summary>
    protected internal SessionState()
    {
    }
    ...
    /// </summary>
    public abstract event EventHandler<SessionStateChangedEventArgs<T>>? Changed;
    ...
    /// </summary>
    public static implicit operator T?(SessionState<T> state) => state.Value;
}
